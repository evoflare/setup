...
PRIMARY_API="https://evoflare-api-dev.herokuapp.com/"
CYAN='\033[0;36m'
NC='\033[0m' # No Color
...
function install() {
    LETS_ENCRYPT="n"
    echo -e -n "${CYAN}(!)${NC} Enter the domain name for your Evoflare instance (ex. evofalre.company.com), default=localhost: "
    read DOMAIN
    echo ""
    ...
    pullSetup
    docker run -it --rm --name setup -v $OUTPUT_DIR:/evoflare \
        --env-file $ENV_DIR/uid.env $DOCKER_REGISTRY/setup:$COREVERSION \
        dotnet Setup.dll -install 1 -domain $DOMAIN -letsencrypt $LETS_ENCRYPT -os $OS \
        -corev $COREVERSION -webv $WEBVERSION -primaryApi $PRIMARY_API -docker-registry $DOCKER_REGISTRY
}
...
function updateDatabase() {
    pullSetup
    docker run -i --rm --name setup --network container:evoflare-mssql \
        -v $OUTPUT_DIR:/evoflare --env-file $ENV_DIR/uid.env evoflare/setup:$COREVERSION \
...
function update() {
    ...
    docker run -i --rm --name setup -v $OUTPUT_DIR:/evoflare \
        --env-file $ENV_DIR/uid.env $DOCKER_REGISTRY/setup:$COREVERSION \
...
function pullSetup() {
    docker pull $DOCKER_REGISTRY/setup:$COREVERSION
}
